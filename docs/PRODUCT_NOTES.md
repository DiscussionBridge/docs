...
[Versions And Live Status](./VERSIONS_AND_LIVE_STATUS.md). Installation behavior
belongs in the [Alpha Installation and Operator Guide](./ALPHA_OPERATOR_GUIDE.md)
...
[Core/Adapter Architecture](./CORE_ADAPTER_ARCHITECTURE.md). How each adapter
creates, updates, withdraws, retries, and recovers a publication is documented
in [Adapter Operating Models](./ADAPTER_OPERATING_MODELS.md).
...
[Presentation Modes](./PRESENTATION_MODES.md) for rendering, authentication,
...
Planned work is labeled in the [Draft Roadmap](./DRAFT_ROADMAP.md). It is not
